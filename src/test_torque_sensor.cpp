...
int main(int argc, char *argv[]) {
    ...
    FILE* fwrite_data = fopen("torque_sensor_data.txt_10", "w");
    ...
    for (int i = 0; i < 100; ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            std::cout << "Joint " << (j + 1) << " Toruqe: " << std::fixed << std::setprecision(4)
                      << single_arm_v1.get_torque(j) << " N/m: " << "Current: " << single_arm_v1.get_current(j) << " A " << std::endl;
            fprintf(fwrite_data, "%.4f %.4f ", single_arm_v1.get_torque(j), single_arm_v1.get_current(j));
        }
        fprintf(fwrite_data, "\n");
